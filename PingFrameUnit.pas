...
uses
  DateUtils, System.Diagnostics, ClipBrd;
...
var
  ...
  LineColor: TColor;
begin
  if lThrowAwayFirstPing then
  begin
//    PingChart.Series[0].AddXY(Now, -1, '', LineColor);
    lThrowAwayFirstPing := False;
  end
  else
  begin
...
procedure TPingFrame.CopyBtnClick(Sender: TObject);
begin
  if Length(Trim(HostEdit.Text)) > 0 then Clipboard.AsText := HostEdit.Text;
end;
